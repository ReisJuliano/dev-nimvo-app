...
Route::prefix('admin')->name('central.admin.')->group(function () {
    ...
    Route::middleware('auth.central_admin:central_admin')->group(function () {
        ...
        Route::get('/tenants/{tenant}/local-agent/commands/{command}', [TenantManagementController::class, 'showLocalAgentCommand'])->name('tenants.local-agent.commands.show');
        Route::get('/tenants/{tenant}/local-agent/download', [TenantManagementController::class, 'downloadLocalAgentInstaller'])->name('tenants.local-agent.download');
