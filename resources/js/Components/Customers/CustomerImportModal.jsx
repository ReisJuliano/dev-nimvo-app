...
const TEMPLATE_HEADERS = 'nome,documento,telefone,email,limite_credito,saldo_fiado_inicial'
const TEMPLATE_SAMPLE = 'Maria da Silva,12345678900,11999990000,maria@example.com,200,50'
...
export default function CustomerImportModal({ open, onClose, onImported }) {
    ...
    async function handleFileChange(event) {
        ...
        try {
            ...
            const response = await apiRequest('/api/customers/import/preview', { method: 'post', data: formData })
    ...
    async function handleConfirm() {
        ...
        try {
            const response = await apiRequest('/api/customers/import/commit', { method: 'post', data: { rows } })
            setResult(response)
            onImported?.()
        } catch (submitError) {
            setError(submitError.message)
    ...
    return (
        <CompactModal
            ...
            description="Envie uma planilha CSV para cadastrar ou atualizar vários clientes de uma vez."
            icon="fa-file-import"
            size="lg"
            onClose={handleClose}
        >
                    ...
                    <>
                        <section className="proc-ui-modal-block">
                            ...
                            <p className="proc-ui-muted">
                                Colunas aceitas: nome*, documento (CPF/CNPJ), telefone, email, limite_credito,
                                saldo_fiado_inicial (dívida já existente do cliente, vira fiado registrado na migração).
                                        ...
                                        <tbody>
                                            {rows.map((row) => (
                                                <tr key={row.row_number}>
                                                    ...
                                                    <td>{row.preview.initial_credit_balance ? formatMoney(row.preview.initial_credit_balance) : '-'}</td>
                                                    <td>
                                                        {row.errors?.length ? (
                        ...
                        <div className="proc-ui-modal-footer">
                            ...
                            >
                                {committing ? 'Importando...' : `Importar ${validRows.length} cliente(s)`}
                            </button>
                        </div>
                    </>
                ) : (
