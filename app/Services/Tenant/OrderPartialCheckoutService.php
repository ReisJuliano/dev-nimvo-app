...
class OrderPartialCheckoutService
{
    ...
    public function checkout(OrderDraft $draft, array $payload, int $userId): array
    {
        ...
        return DB::transaction(function () use ($draft, $payload, $userId, $cashRegister) {
            ...
            /** @var Collection<int,array{id:int,qty:float,discount:float}> $itemsPayload */
            ...
            if ($itemsPayload->isEmpty()) {
                throw ValidationException::withMessages([
                    'items' => 'Selecione ao menos um item para cobrar.',
                ]);
            }

            $draftItemsByProduct = $orderDraft->items->keyBy('product_id');
            foreach ($itemsPayload as $item) {
                /** @var OrderDraftItem|null $draftItem */
            ...
            $items = $itemsPayload->map(function (array $item) use ($products) {
                /** @var Product $product */
            ...
            foreach ($itemsPayload as $item) {
                /** @var OrderDraftItem $draftItem */
                $draftItem = $draftItemsByProduct->get($item['id']);
                $remaining = max(0, round((float) $draftItem->quantity - (float) $item['qty'], 3));
                ...
                $draftItem->forceFill([
                    'quantity' => $remaining,
                    'total' => round((float) $draftItem->unit_price * $remaining, 2),
                ])->save();
            }

            $orderDraft->refresh();
            $orderDraft->load(['items']);

            if ($orderDraft->items->isEmpty()) {
                $this->orderDraftService->markAsCompleted($orderDraft, $sale);
                $orderDraft->refresh();
            } else {
                $remainingSubtotal = round((float) $orderDraft->items->sum('total'), 2);
                $remainingCost = round((float) $orderDraft->items->sum(fn (OrderDraftItem $item) => (float) $item->unit_cost * (float) $item->quantity), 2);
                $orderDraft->forceFill([
                    'subtotal' => $remainingSubtotal,
                    'total' => $remainingSubtotal,
                    'cost_total' => $remainingCost,
                    'profit' => round($remainingSubtotal - $remainingCost, 2),
                ])->save();
            }
            ...
            return [
                ...
                'order' => $orderDraft->fresh(['customer:id,name,phone', 'user:id,name', 'items' => fn ($query) => $query->with('product:id,stock_quantity')->orderBy('id')]),
            ];
        });
    }
